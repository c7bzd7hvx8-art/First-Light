...
export var GROUND_MARKER_TYPES = Object.freeze([
  ...
  // 12.85 (Stalking Directory request): feed sites are their own furniture —
  // a hopper and a mineral lick are different objects on the ground and both
  // are where cameras end up pointing.
  { id: 'feeder',    label: 'Feeder' },
  { id: 'lick',      label: 'Mineral lick', chip: 'Lick' },
...
export function makeMarkerGeometry(lat, lng, mtype, extra) {
  var ring = normalizeRing([[lat, lng]]);
  var known = GROUND_MARKER_TYPES.some(function(t) { return t.id === mtype; });
  var g = { v: 1, ring: ring, mt: known ? mtype : 'other' };
  // 12.85: optional extras ride in the blob like mt does — no DB migration.
  // fc = the direction a trail camera points (degrees, 8-way). ck = the date
  // its card/batteries were last checked (local YYYY-MM-DD). Anything invalid
  // is simply not written, so old readers and old blobs are both untouched.
  if (extra && typeof extra === 'object') {
    var fc = extra.facing;
    if (Number.isFinite(fc) && fc >= 0 && fc < 360) g.fc = Math.round(fc);
    var ck = extra.checked;
    if (typeof ck === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(ck)) g.ck = ck;
  }
  return g;
}
...
export function markerFromGeometry(geom) {
  ...
  var fc = (Number.isFinite(geom.fc) && geom.fc >= 0 && geom.fc < 360) ? Math.round(geom.fc) : null;
  var ck = (typeof geom.ck === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(geom.ck)) ? geom.ck : null;
  return { lat: round6(p[0]), lng: round6(p[1]), type: known ? mt : 'other', facing: fc, checked: ck };
}
      ...
      out.features.push({
        ...
        properties: withExtras(
          ...
          { marker_type_label: markerTypeLabel(mk.type), notes: f.notes || null,
            facing: mk.type === 'trail_cam' ? mk.facing : null, // 12.87
            checked: mk.type === 'trail_cam' ? mk.checked : null }
        ),
 ...
 */
/** 12.87: a trail cam's aim + last check ride along in every export — desc
 *  for human/handheld eyes, ext/properties for our own round-trip ('facing'
 *  was already an import key for seats; 'checked' joins it). */
function camExtras(mk) {
  var out = { desc: [], ext: {} };
  if (!mk || mk.type !== 'trail_cam') return out;
  if (mk.facing != null) {
    out.desc.push('Points ' + ['N','NE','E','SE','S','SW','W','NW'][Math.round(mk.facing / 45) % 8]);
    out.ext.facing = mk.facing;
  }
  if (mk.checked) { out.desc.push('Checked ' + mk.checked); out.ext.checked = mk.checked; }
  return out;
}
...
export function featuresToGpx(features, stands) {
  ...
  (features || []).forEach(function(f) {
    ...
    if (f.kind === 'marker') { // G10: markers are waypoints, like the seats
      ...
      var cx = camExtras(mk); // 12.87: aim + last check ride along
      var mdesc = descLine([(f.ground || '') + (f.ground ? ' — ' : '') + tlabel].concat(cx.desc).concat([f.notes || '']));
      var mext = {
        ground: f.ground || '', kind: 'marker', marker_type: mk.type,
        name: f.name || '', notes: f.notes || ''
      };
      if (cx.ext.facing != null) mext.facing = cx.ext.facing;
      if (cx.ext.checked) mext.checked = cx.ext.checked;
...
export function featuresToKml(features, stands) {
  ...
  (features || []).forEach(function(f) {
    ...
    if (f.kind === 'marker') {
      ...
      var tlabel = markerTypeLabel(mk.type);
      var cx = camExtras(mk); // 12.87: aim + last check ride along
      var kext = { ground: f.ground || '', kind: 'marker', marker_type: mk.type, name: f.name || '', notes: f.notes || '' };
      if (cx.ext.facing != null) kext.facing = cx.ext.facing;
      if (cx.ext.checked) kext.checked = cx.ext.checked;
      folders[bucket(f.ground)] += kmlPlacemark(
        f.name || tlabel,
        descLine([tlabel].concat(cx.desc).concat([f.notes || ''])),
        'fl-marker',
        kext,
...
var FL_EXT_KEYS = ['ground', 'kind', 'name', 'notes', 'line_type', 'marker_type',
                   'facing', 'bad_winds', 'checked']; // 12.87: cam check date
...
function flDateStr(v) { return (typeof v === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(v)) ? v : null; } // 12.87
function flList(v) {
...
function importFeature(kind, ring, o) {
  ...
  return {
    ...
    checked: flDateStr(o.checked), // 12.87: cam last-check survives import
...
export function parseGpxFeatures(text) {
  ...
  eachElem(s, 'wpt', function(inner, attrs) {
    ...
    out.push(importFeature(kind, pt, {
      ...
      checked: flDateStr(ext.checked), // 12.87
    ...
    if (gt === 'Polygon' || gt === 'MultiPolygon') {
      ...
      (Array.isArray(pts) ? pts : []).forEach(function(c) {
        ...
        out.push(importFeature(k, r, optsWith(base, {
          ...
          checked: flDateStr(pr.checked), // 12.87
