...
// Maximum bullet-weight difference (grains) between a source load and a
// lead-free candidate. Wider than this and the candidate is not a credible
// "like-for-like" alternative — terminal performance, recoil impulse and
// twist-rate requirements diverge too far.
//
// Calibration (2026-06-09): chosen so a stalker using a 156-162gr lead load
// in 6.5×55 / .270 Win / 7×57 is NOT silently offered a 93-130gr lead-free
// "alternative" (the inverse-scoring matcher would otherwise rank that
// drop-and-energy match as best). ±25gr keeps deer-class options in the
// pool: e.g. a 150gr lead can still match a 130gr copper monolithic. If
// the candidate pool empties to zero on weight grounds, the matcher
// returns reason='no-weight-match' so the UI can surface a calibre-
// change advisory rather than silently pretending nothing was available.
export const WEIGHT_BAND_GRAINS = 25;

// Constructions that denote a genuinely lead-free projectile. A candidate is
// only offered as a lead-free alternative when its `leadFree` flag AND its
// construction agree — belt-and-braces, so a single mislabelled record (a lead
// bullet flagged leadFree:true, which shipped in the DB and made the matcher
// recommend a LEAD bullet as the top "lead-free" substitute) can never again be
// recommended. Allowlist rather than a lead denylist, because a lead-free
// deformation bullet can carry an ambiguous construction tag; excluding an
// unknown tag fails safe (never suggests lead) and is fixed by tagging the load.
export const LEAD_FREE_CONSTRUCTIONS = new Set([
  'monolithic-copper', 'copper', 'solid-copper', 'gilding-metal', 'tin', 'lead-free'
]);

/** True only when a load is flagged lead-free AND has a lead-free construction. */
export function isTrulyLeadFree(load) {
  return !!load && load.leadFree === true &&
    LEAD_FREE_CONSTRUCTIONS.has(String(load.construction || '').toLowerCase());
}
 ...
 * @param {number} [opts.weightBandGrains=WEIGHT_BAND_GRAINS]
 *        Override the like-for-like weight band. Pass Infinity to disable
 *        the filter (useful in tests).
 * @returns {{
 *   reason: 'ok' | 'already-lead-free' | 'no-alternatives' | 'no-weight-match' | 'invalid-source',
 *   matches: Array<{
 *     load: object,
 *     score: number,
 *     deltas: Array,
...
export function findLeadFreeAlternatives(db, sourceLoad, opts = {}) {
  ...
  const weightBand = opts.weightBandGrains ?? WEIGHT_BAND_GRAINS;
  ...
  const sameCalibre = (db.loads || []).filter(l =>
    l.calibre === sourceLoad.calibre &&
    isTrulyLeadFree(l) &&
    l.id !== sourceLoad.id
  );

  if (sameCalibre.length === 0) {
    return {
  ...
  // Apply the like-for-like weight band. Without this, the scoring path
  // happily returns wildly under-weighted candidates because drop and
  // energy at 100-300m can look similar across very different bullet
  // masses. See WEIGHT_BAND_GRAINS for the calibration rationale.
  const sourceWeight = Number(sourceLoad.weightGrains);
  const candidates = Number.isFinite(sourceWeight)
    ? sameCalibre.filter(l =>
        Number.isFinite(l.weightGrains) &&
        Math.abs(l.weightGrains - sourceWeight) <= weightBand)
    : sameCalibre;

  if (candidates.length === 0) {
    return {
      reason: 'no-weight-match',
      matches: [],
      sourceTrajectory: sourceTraj,
    };
  }
