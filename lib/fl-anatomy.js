 ...
 * }}
 */
export function getAnatomicalHold({ dropCm, speciesKey, sex, aimPointKey, presentation }) {
  ...
  if (Math.abs(dropCm) > vital * 1.5 && !warning) {
    warning = pres.vitalFactor < 1
      ? `Hold offset (${Math.abs(dropCm).toFixed(0)} cm) approaches the vital zone, which is narrower on a ${pres.label.toLowerCase()} deer — a small ranging error is costly at this angle and distance.`
      : `Hold offset (${Math.abs(dropCm).toFixed(0)} cm) approaches or exceeds the vital zone — small ranging error has a large impact at this distance.`;
  }
