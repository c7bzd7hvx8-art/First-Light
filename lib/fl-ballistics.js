...
export function maxPointBlankRange(p, vitalRadiusCm) {
  ...
  // 12.88 (user email: a metric PBR calculator): the optimiser now reports
  // everything the sight-in instruction needs — the rise at exactly 100 m
  // (the number a UK stalker dials on the range), the near/far zeros, and
  // the arc's peak. All additive; existing callers see the same fields.
  let near = null, far = null, zero = null, nearZero = null, prev = null;
  let maxRiseCm = 0, riseRangeM = null, sightIn100 = null;
  for (const r of t) {
    const rise = -r.dropCm;
    if (rise > maxRiseCm) { maxRiseCm = rise; riseRangeM = r.rangeM; }
    if (r.dropCm <= vitalRadiusCm) { if (near == null) near = r.rangeM; far = r.rangeM; }
    if (prev && prev.dropCm > 0 && r.dropCm <= 0) nearZero = r.rangeM;
    if (prev && prev.dropCm < 0 && r.dropCm >= 0) zero = r.rangeM;
    if (r.rangeM === 100) sightIn100 = rise;
    prev = r;
  }
  ...
  return {
    ...
    vitalRadiusCm,
    nearZeroM: nearZero != null ? Math.round(nearZero) : null,
    sightIn100Cm: sightIn100 != null ? Math.round(sightIn100 * 10) / 10 : null,
...
/**
 * 12.99 (Ian, by email: "without a safe backstop... this bullet has the
 * potential to travel 2700m if uninterrupted"): the maximum distance this
 * load can CARRY — launch-angle sweep over the same drag integration as
 * every other figure here, ground taken as ~1.7 m below a shoulder-height
 * muzzle. An estimate by nature (bullets destabilise in descent, and the
 * flat-fire drag model is stretched at high elevation), which is why the UI
 * says "roughly" and rounds to 100 m — but it is THIS load's number, not a
 * stock phrase. Published figures for a .308 150gr sit around 4–4.6 km;
 * this lands inside that band.
 *
 * @returns {number|null} metres, or null on unusable inputs.
 */
export function maxCarryRangeM(p) {
  if (!(p.muzzleVelocityMs > 0)) return null;
  if (!(p.bcG1 > 0) && !(p.bcG7 > 0)) return null;
  const densityRatio = airDensityRatio(p.tempC, p.pressureHpa, p.humidityPct);
  const base = {
    muzzleVelocityMs: p.muzzleVelocityMs, bcG1: p.bcG1, bcG7: p.bcG7,
    bulletMassKg: p.bulletMassKg, sightHeightCm: p.sightHeightCm || 4,
    densityRatio, tempC: p.tempC,
  };
  const GROUND_DROP_CM = 170;   // muzzle ~1.7 m above the ground it lands on
  const carryAt = (deg) => {
    const t = solveTrajectory({
      ...base, launchAngleRad: deg * Math.PI / 180,
      maxRangeM: 8000, stepM: 10, dt: 0.001,
    });
    let past = 0;
    for (const r of t) {
      if (r.dropCm >= GROUND_DROP_CM) { past = r.rangeM; break; }
      past = r.rangeM;
    }
    return past;
  };
  let bestDeg = 0, best = 0;
  for (let d = 10; d <= 45; d += 5) {
    const c = carryAt(d);
    if (c > best) { best = c; bestDeg = d; }
  }
  for (const d of [bestDeg - 2.5, bestDeg + 2.5]) {
    const c = carryAt(d);
    if (c > best) best = c;
  }
