...
async function handleAuth() {
  ...
  try {
    ...
    if (authMode === 'signin') {
      ...
      // After email confirmation, land on the diary (not the main site home).
      var diaryUrl = window.location.origin + '/diary.html';
      result = await sb.auth.signUp({
        email: email,
        password: password,
        options: {
          data: { full_name: name },
          emailRedirectTo: diaryUrl
        }
      });
    }
    if (result.error) throw result.error;
...
async function openSummaryFilter() {
  ...
  if (currentSeason && currentSeason !== '__all__') {
